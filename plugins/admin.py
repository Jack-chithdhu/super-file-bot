...
async def handle_join_request(client: Client, request):
    """Save join request to DB so bot can grant access immediately."""
    try:
        from database.database import add_join_request
        await add_join_request(request.from_user.id)
    except Exception as e:
        import logging
        logging.getLogger(__name__).warning(f"Join request save error: {e}")
