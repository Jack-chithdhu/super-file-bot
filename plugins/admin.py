...
async def handle_join_request(client: Client, request):
    """
    Handle join requests:
    - Mode 2 (Access on Request): save to DB, bot grants access immediately
    - Mode 3 (Auto Approve): bot approves request automatically
    """
    try:
        from database.database import add_join_request, get_setting
        user_id = request.from_user.id

        # Always save to DB (Mode 2 — access on request)
        await add_join_request(user_id)

        # Mode 3 — Auto Approve
        auto_approve = await get_setting('join_request_auto_approve') or False
        if auto_approve:
            # Approve — add user to channel
            try:
                ...
                import logging
                logging.getLogger(__name__).warning(f"Auto approve error: {ae}")
        else:
            # Decline — user stays out of channel but has bot access via DB
            try:
                ...
                import logging
                logging.getLogger(__name__).warning(f"Decline join request error: {de}")
...
# ─────────────────────────────────────────────────────────────────────────────
#  /clearjoinrequests — wipe all stale pending join requests from DB
# ─────────────────────────────────────────────────────────────────────────────
@Bot.on_message(filters.command('clearjoinrequests') & filters.private & filters.user(ADMINS))
async def clear_join_requests(client: Client, message: Message):
    from pymongo import MongoClient as _MC
    from config import DB_URI, DB_NAME
    db = _MC(DB_URI)[DB_NAME]['join_requests']
    result = db.delete_many({'status': 'pending'})
    await message.reply(
        f"✅ Cleared <b>{result.deleted_count}</b> stale pending join requests from DB.\n\n"
        f"Users will now need to send a new join request to get access."
    )
