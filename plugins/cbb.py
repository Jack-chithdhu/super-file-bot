...
async def cb_handler(client: Bot, query: CallbackQuery):
    ...
    # Answer immediately to stop button glow
    await query.answer()
    ...
    # ── Buy Premium ───────────────────────────────────────────────────────
        ...
        days   = await _gs('premium_duration') or 30

        text = (
            f"💎 <b>GET PREMIUM</b>\n\n"
            f"💰 <b>₹{price}</b> for {days} days\n\n"
            f"📱 UPI: <code>{upi}</code>\n"
            f"1️⃣ Pay via UPI or scan QR\n"
            f"2️⃣ Tap ✅ <b>I've Paid</b>\n"
            f"3️⃣ Send screenshot to admin\n"
            f"4️⃣ Admin activates premium! ✨"
        )
        ...
        if qr_file_id:
            try:
                # Send QR as a new message then delete old one
