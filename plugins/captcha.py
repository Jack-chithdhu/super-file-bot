...
captcha_just_passed = set()  # users who just passed, bypass start check once
...
async def captcha_callback(client: Client, query: CallbackQuery):
    ...
    try:
        ...
        # Send silently then delete immediately before calling start_command
        sent = await client.send_message(
            chat_id=user_id,
            text=fake_text,
            disable_notification=True
        )
        # Delete immediately so user never sees it
        try:
            await sent.delete()
        except Exception:
            pass
        # Now call start_command with the message object
