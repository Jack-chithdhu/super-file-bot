...
from database.database import (
    ...
    link_collection_to_request, get_collection_for_request
)
...
async def getfiles_cmd(client: Client, message: Message):
    ...
    req_id = args[0].strip().upper()

    # Look up linked collection number for this request ID
    col_number = await get_collection_for_request(req_id)
    if col_number is None:
        await message.reply(f"❌ No files linked to request <code>{req_id}</code>.\n\n<i>Admin has not linked files yet.</i>")
        return

    # Use collection number as storage key
    storage_id = str(col_number)
    col = await collection_get_by_number(col_number)
    display_title = col["name"] if col else f"Collection #{col_number}"

    seasons = await getfiles_get_seasons(storage_id)
    if not seasons:
        await message.reply(f"❌ No files found for <code>{req_id}</code>.\n\n<i>Admin may not have uploaded files yet.</i>")
        return

    buttons = [[InlineKeyboardButton(f"📅 Season {s}", callback_data=f"gf_season_{storage_id}_{s}")] for s in seasons]
    buttons.append([InlineKeyboardButton("🔒 Close", callback_data="close")])

    req = await get_request_by_id(req_id)
    title = req["title"] if req else display_title
...
# ─────────────────────────────────────────────────────────────────────────────
#  /linkfile <collection_number> <request_id>  — admin links collection to request
# ─────────────────────────────────────────────────────────────────────────────
@Bot.on_message(filters.command("linkfile") & filters.private & filters.user(ADMINS))
async def linkfile_cmd(client: Client, message: Message):
    args = message.command[1:]
    if len(args) < 2:
        await message.reply(
            "ℹ️ <b>Usage:</b> <code>/linkfile &lt;collection_number&gt; &lt;request_id&gt;</code>\n\n"
            "<i>Example: /linkfile 1 7GE0H6</i>"
        )
        return
    ...
    try:
        ...
        await message.reply("❌ Collection number must be a number. Example: /linkfile 1 7GE0H6")
        return

    req_id = args[1].strip().upper()
    ...
    await link_collection_to_request(col_number, req_id)

    req = await get_request_by_id(req_id)
    req_title = req["title"] if req else req_id

    await message.reply(
        f"✅ <b>Linked!</b>\n\n"
        f"📦 Collection <b>#{col_number} — {col['name']}</b>\n"
        f"🔗 Request <code>{req_id}</code> — <b>{req_title}</b>\n\n"
        f"Users can now use:\n<code>/getfiles {req_id}</code>"
    )
...
# ─────────────────────────────────────────────────────────────────────────────
#  Get Files button callback — triggered from fulfill notification
# ─────────────────────────────────────────────────────────────────────────────
@Bot.on_callback_query(filters.regex(r"^gf_req_(.+)$"))
async def gf_req_cb(client: Client, query: CallbackQuery):
    req_id = query.data[7:]

    col_number = await get_collection_for_request(req_id)
    if col_number is None:
        await query.answer("❌ No files linked to this request yet.", show_alert=True)
        return

    storage_id = str(col_number)
    col        = await collection_get_by_number(col_number)
    req        = await get_request_by_id(req_id)
    title      = req["title"] if req else (col["name"] if col else req_id)

    seasons = await getfiles_get_seasons(storage_id)
    if not seasons:
        await query.answer("❌ No files uploaded yet.", show_alert=True)
        return
...
# ─────────────────────────────────────────────────────────────────────────────
#  /linkcollection <number> <request_id>  — link a collection to a request
# ─────────────────────────────────────────────────────────────────────────────
@Bot.on_message(filters.command("linkcollection") & filters.private & filters.user(ADMINS))
async def linkcollection_cmd(client: Client, message: Message):
    args = message.command[1:]
    if len(args) < 2:
        await message.reply(
            "ℹ️ <b>Usage:</b> <code>/linkcollection &lt;number&gt; &lt;request_id&gt;</code>\n\n"
            "<i>Example: /linkcollection 1 7GE0H6</i>"
        )
        return
    ...
    try:
        ...
        await message.reply("❌ Collection number must be a number.")
        return
    ...
    await message.reply(
        f"✅ <b>Linked!</b>\n\n"
        f"📦 Collection <b>#{col_number}</b> — {col['name']}\n"
        f"🔗 → Request <code>{req_id}</code> — <b>{req['title']}</b>\n\n"
        f"Users with request ID <code>{req_id}</code> can now use\n"
        f"<code>/getfiles {req_id}</code> to get their files."
    )


# ─────────────────────────────────────────────────────────────────────────────
#  gf_req_<request_id>  — Get Files button from fulfill notification
# ─────────────────────────────────────────────────────────────────────────────
@Bot.on_callback_query(filters.regex(r"^gf_req_(.+)$"))
async def gf_req_cb(client: Client, query: CallbackQuery):
    req_id     = query.data[7:]
    col_number = await get_collection_for_request(req_id)
    if col_number is None:
        await query.answer("❌ Files not linked yet. Contact admin.", show_alert=True)
        return
    ...
    if not seasons:
        await query.answer("❌ No files uploaded yet. Contact admin.", show_alert=True)
        return
