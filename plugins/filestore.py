...
async def fs_collections(client: Client, query: CallbackQuery):
    ...
    # Show collections with their numbers from getfiles system
    from database.database import collection_get_all
    col_numbers = await collection_get_all()
    col_num_map = {c["name"]: c["number"] for c in col_numbers}

    buttons = []
    for c in collections:
        num = col_num_map.get(c)
        label = f"#{num} — {c}" if num else f"📦 {c}"
        buttons.append([InlineKeyboardButton(label, callback_data=f"fs_coll_{c}")])
    buttons.append([InlineKeyboardButton("🔙 Back", callback_data="fs_home")])

    await query.message.edit_text(
        f"📦 <b>Collections</b> ({len(collections)} total)\n\nNumbers are used with /getfiles and /addfile:\n",
        reply_markup=InlineKeyboardMarkup(buttons)
    )
    await query.answer()
