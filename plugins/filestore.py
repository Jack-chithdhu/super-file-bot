...
# Handle new collection name input
@Bot.on_message(filters.private & filters.user(ADMINS) & filters.text & ~filters.command([
    'start','filestore','cancel','batch','genlink'
]))
async def fs_new_coll_input(client: Client, message: Message):
    user_id = message.from_user.id
    if not hasattr(client, '_fs_gen') or user_id not in client._fs_gen:
        return
    state = client._fs_gen[user_id]
    if state.get('step') != 'new_coll':
        return
