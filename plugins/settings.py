...
async def show_main_settings(client, chat_id, message_id=None):
    ...
    survival_on = await get_setting('survival_mode')
    if survival_on is None: survival_on = False
    ...
    markup = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"⚡ SURVIVAL MODE  {surv_icon}", callback_data="cfg_survival")],
...
async def cfg_set_dailylimit(client, query: CallbackQuery):
    ...
    await query.answer()
    try:
        resp = await client.ask(query.from_user.id, "📝 Send your reply:", filters=filters.text, timeout=60)
...
async def cfg_payment(client, query: CallbackQuery):
    ...
    await query.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("💰 Set Price",            callback_data="cfg_pay_price")],
...
@Bot.on_callback_query(filters.regex(r"^cfg_pay_price$") & filters.user(OWNER_ID))
async def cfg_pay_price(client, query):
    await query.message.edit_text(
        "💰 <b>Set Premium Price</b>\n\n"
        "Send the price amount (numbers only).\n\n"
        "<i>Example: 99</i>",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("❌ Cancel", callback_data="cfg_payment")
        ]])
    ...
    try:
        resp = await client.ask(query.from_user.id, "📝 Send your reply:", filters=filters.text, timeout=60)
        price = int(resp.text.strip())
        await set_setting('premium_price', price)
        await resp.reply(f"✅ Premium price set to ₹{price}")
    except (asyncio.TimeoutError, ValueError):
        await client.send_message(query.from_user.id, "❌ Cancelled or invalid. Please enter numbers only.")
...
async def cfg_pay_qr(client, query: CallbackQuery):
    ...
    await query.answer()
    # Set a flag so channel_post knows we are waiting for QR photo
    if not hasattr(client, 'waiting_qr'):
        client.waiting_qr = set()
    client.waiting_qr.add(query.from_user.id)
    try:
        resp = await client.ask(
            query.from_user.id, "📷 Send the QR photo:",
            filters=filters.photo, timeout=60
        )
        if resp and resp.photo:
            file_id = resp.photo.file_id
            await set_payment_qr(file_id)
            await client.send_message(query.from_user.id, "✅ QR image saved!")
        else:
            await client.send_message(query.from_user.id, "❌ No photo received. Try again.")
    except asyncio.TimeoutError:
        ...
        if hasattr(client, 'waiting_qr'):
            client.waiting_qr.discard(query.from_user.id)
    await show_main_settings(client, query.from_user.id)
...
async def cfg_captcha_reset_do(client, query: CallbackQuery):
    ...
    captcha_module.captcha_just_passed.clear()

    # Clear DB-persisted captcha_passed entries (multi-worker safe)
    from database.database import reset_all_captcha_passed
    await reset_all_captcha_passed()
...
async def cfg_req_custmsg_edit(client, query):
    ...
    await query.answer()
    try:
        resp = await client.ask(query.from_user.id, "📝 Send your reply:", filters=filters.text, timeout=120)
...
async def cfg_forcesub(client, query: CallbackQuery):
    ...
    jr_auto      = await get_setting('join_request_auto_approve') or False
    icon         = "✅ ON" if fs_ch and fs_ch != 0 else "❌ OFF"
    jr_icon      = "✅ ON" if jr_on else "❌ OFF"
    access_icon  = "✅ ON" if jr_access else "❌ OFF"
    auto_icon    = "✅ ON" if jr_auto else "❌ OFF"
    ...
    await query.message.edit_text(
        ...
        f"🔓 <b>Access on Request:</b> {access_icon}\n"
        f"✅ <b>Auto Approve:</b> {auto_icon}\n\n"
        "💎 Premium users always bypass force subscribe.\n\n"
        "<b>Modes:</b>\n"
        "🔓 <b>Access on Request</b> — user gets bot access immediately after sending join request\n"
        "✅ <b>Auto Approve</b> — bot auto-approves join requests and adds user to channel",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("📢 Set Channel ID",         callback_data="cfg_set_fschannel")],
            [
                InlineKeyboardButton("🟢 Enable Force Sub",   callback_data="cfg_enable_forcesub") if not (fs_ch and fs_ch != 0)
                else InlineKeyboardButton("🔴 Disable Force Sub", callback_data="cfg_disable_forcesub")
            ],
            [InlineKeyboardButton(
            ...
            [InlineKeyboardButton(
                f"🔓 Access on Request: {access_icon}",
                callback_data="cfg_toggle_jr_access"
            )],
            [InlineKeyboardButton(
                f"✅ Auto Approve: {auto_icon}",
                callback_data="cfg_toggle_jr_auto"
            )],
            [InlineKeyboardButton("🔴 Reset All Force Sub",    callback_data="cfg_reset_forcesub")],
            [InlineKeyboardButton("🔙 BACK",                   callback_data="cfg_back")],
        ])
    )
    await query.answer()
...
@Bot.on_callback_query(filters.regex(r"^cfg_enable_forcesub$") & filters.user(OWNER_ID))
async def cfg_enable_forcesub(client, query):
    await query.message.edit_text(
        "📢 <b>Enable Force Subscribe</b>\n\n"
    ...
    await query.answer()
    try:
        resp  = await client.ask(query.from_user.id, "📝 Send channel ID:", filters=filters.text, timeout=60)
        ch_id = int(resp.text.strip())
        await set_setting('force_sub_channel', ch_id)
        await resp.reply(f"✅ Force subscribe enabled! Channel: <code>{ch_id}</code>")
    except (asyncio.TimeoutError, ValueError):
        await client.send_message(query.from_user.id, "❌ Cancelled or invalid.")
    await show_main_settings(client, query.from_user.id)


@Bot.on_callback_query(filters.regex(r"^cfg_reset_forcesub$") & filters.user(OWNER_ID))
async def cfg_reset_forcesub(client, query):
    # Reset ALL force sub related settings
    await set_setting('force_sub_channel', 0)
    await set_setting('join_request_enabled', False)
    await set_setting('join_request_access', False)
    await set_setting('join_request_auto_approve', False)
    await query.answer("✅ All force sub settings reset!", show_alert=True)
...
@Bot.on_callback_query(filters.regex(r"^cfg_toggle_jr_access$") & filters.user(OWNER_ID))
async def cfg_toggle_jr_access(client, query):
    current = await get_setting('join_request_access') or False
    new_val = not current
    await set_setting('join_request_access', new_val)
    icon = "✅ ON" if new_val else "❌ OFF"
    await query.answer(f"Access on Request: {icon}", show_alert=True)
    await cfg_forcesub(client, query)


@Bot.on_callback_query(filters.regex(r"^cfg_toggle_jr_auto$") & filters.user(OWNER_ID))
async def cfg_toggle_jr_auto(client, query):
    current = await get_setting('join_request_auto_approve') or False
    new_val = not current
    await set_setting('join_request_auto_approve', new_val)
    icon = "✅ ON" if new_val else "❌ OFF"
    await query.answer(f"Auto Approve: {icon}", show_alert=True)
    await cfg_forcesub(client, query)
...
async def cfg_set_welcomepic(client, query):
    ...
    await query.answer()
    try:
        resp    = await client.ask(query.from_user.id, "📷 Send the photo:", filters=filters.photo, timeout=60)
        file_id = resp.photo.file_id
...
# ── ⚡ Survival Mode ──────────────────────────────────────────────────────────
@Bot.on_callback_query(filters.regex(r"^cfg_survival$") & filters.user(OWNER_ID))
async def cfg_survival(client, query: CallbackQuery):
    current = await get_setting('survival_mode') or False
    icon = "✅ ON" if current else "❌ OFF"
    await query.message.edit_text(
        "╔══════════════════════════╗\n"
        "    ⚡  <b>SURVIVAL MODE</b>\n"
        "╚══════════════════════════╝\n\n"
        f"🔘 <b>Status:</b> {icon}\n\n"
        "When <b>ON</b> — only file delivery works.\n"
        "Captcha, support, requests, premium checks\n"
        "are all disabled to save resources.\n\n"
        "💡 Enable when Railway credits are low.",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton(
                "🔴 Turn OFF" if current else "🟢 Turn ON",
                callback_data="cfg_survival_toggle"
            )],
            [InlineKeyboardButton("🔙 BACK", callback_data="cfg_back")],
        ])
    )
...
@Bot.on_callback_query(filters.regex(r"^cfg_survival_toggle$") & filters.user(OWNER_ID))
async def cfg_survival_toggle(client, query: CallbackQuery):
    current = await get_setting('survival_mode') or False
    new_val = not current
    await set_setting('survival_mode', new_val)
    icon = "✅ ON" if new_val else "❌ OFF"
    await query.answer(f"⚡ Survival Mode: {icon}", show_alert=True)
    await cfg_survival(client, query)
...
@Bot.on_message(filters.private & filters.user([OWNER_ID]) & ~filters.command(["settings","start","help","users","broadcast","ban","unban","stats","announce","addadmin","removeadmin","listadmins","togglepremium","setdailylimit","adminhelp","addpremium","removepremium","listpremium","allrequests","fulfill","decline","filestats","activechats","chatto","endchat","batch","genlink","setpayment","setqr"]), group=-2)
