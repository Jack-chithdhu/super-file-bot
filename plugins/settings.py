...
async def cfg_forcesub(client, query: CallbackQuery):
    ...
    jr_access    = await get_setting('join_request_access') or False   # Mode 2: access on request
    jr_auto      = await get_setting('join_request_auto_approve') or False  # Mode 3: auto approve
...
async def cfg_toggle_jr_access(client, query):
    ...
    # Clear all pending join requests on every toggle — prevents stale records
    # from granting unintended access when turning on, and cleans up when turning off
    from pymongo import MongoClient as _MC
    from config import DB_URI, DB_NAME
    _MC(DB_URI)[DB_NAME]['join_requests'].delete_many({'status': 'pending'})
    icon = "✅ ON" if new_val else "❌ OFF"
    await query.answer(f"Access on Request: {icon} — pending requests cleared", show_alert=True)
    await cfg_forcesub(client, query)
