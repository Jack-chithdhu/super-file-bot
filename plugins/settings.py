...
async def cfg_captcha_reset_do(client, query: CallbackQuery):
    # Clear verified_users set in captcha module
    from plugins.captcha import verified_users, captcha_store
    count = len(verified_users)
    verified_users.clear()
    captcha_store.clear()

    # Also save a reset timestamp so persistent users are also reset
