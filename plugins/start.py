...
async def start_command(client: Client, message: Message):
    ...
    if base64_payload:
        ...
        # Track filestore link stats if this link is a saved named link
        try:
            from plugins.filestore import _col, _record_access
            import asyncio as _aio
            _loop = _aio.get_event_loop()
            _link_url = f"https://t.me/{client.username}?start={base64_string}"
            _match = await _loop.run_in_executor(None, lambda: _col().find_one({'link': _link_url}))
            if _match:
                await _record_access(_match['link_id'], user_id)
        except Exception:
            pass
