...
async def not_joined(client: Client, message: Message):
    ...
    # Always add Try Again button with start payload
    try:
    ...
    # Custom message for join request mode
    if jr_access or join_req_mode:
        force_text = (
            f"👋 <b>Hey {message.from_user.mention}!</b>\n\n"
            f"🔒 <b>Access Restricted</b>\n\n"
            f"Tap <b>Join Channel</b> to send a join request.\n"
            f"Once sent, tap <b>Try Again</b> to access the bot.\n\n"
            f"<i>You don't need to be approved — just send the request!</i>"
        )
        await message.reply(
            text=force_text,
            reply_markup=InlineKeyboardMarkup(buttons),
