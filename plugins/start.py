...
async def not_joined(client: Client, message: Message):
    ...
    if jr_access or join_req_mode or bool(JOIN_REQUEST_ENABLE):
        # Join Request mode — create a join request link
        try:
        ...
        # Show join request instructions
        try:
        ...
        buttons_jr = [
            ...
            [InlineKeyboardButton("✅ I Sent Request", callback_data=f"jr_done_{message.from_user.id}_{payload}")]
        ]
        await message.reply(
        ...
        # Normal mode — use regular invite link
...
async def jr_done_callback(client, query):
    ...
    # Parse payload — format: jr_done_{user_id}_{payload}
    parts   = query.data.split("_", 3)
    payload = parts[3] if len(parts) > 3 else "start"

    # Save to DB — grant bot access
    from database.database import add_join_request
    await add_join_request(user_id)

    start_url = f"https://t.me/{client.username}?start={payload}"
    await query.message.edit_text(
        "✅ <b>Access Granted!</b>\n\n"
        "Your join request has been received.\n"
        "Tap below to start using the bot!",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("🚀 Start Bot", url=start_url)
        ]])
    )
