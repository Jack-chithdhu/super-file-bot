...
async def start_command(client: Client, message: Message):
    ...
    if base64_payload:
        # ── Force Subscribe check before file delivery ─────────────────────
        force_sub = await get_setting('force_sub_channel') or FORCE_SUB_CHANNEL
        if force_sub and user_id not in ADMINS and not user_is_premium:
            ...
            try:
                ...
                # User not subscribed — show join button
                try:
                    jr_mode   = await get_setting('join_request_enabled') or False
                    jr_access = await get_setting('join_request_access') or False
                    if jr_mode or jr_access:
                        # Send a join REQUEST link so bot can detect and grant access
                        inv = await client.create_chat_invite_link(
                            chat_id=int(force_sub), creates_join_request=True)
                        invite_link = inv.invite_link
                    else:
                        invite_link = (await client.get_chat(int(force_sub))).invite_link
                except Exception:
                    invite_link = f"https://t.me/c/{str(force_sub).replace('-100','')}"
                buttons = [
                    [InlineKeyboardButton("📢 Join Channel", url=invite_link)],
                    [InlineKeyboardButton("🔄 Try Again", url=f"https://t.me/{client.username}?start={base64_payload}")]
                ]
                await message.reply(
...
async def not_joined(client: Client, message: Message):
    # Read force sub channel dynamically from DB
    force_sub = await get_setting('force_sub_channel') or FORCE_SUB_CHANNEL
    if not force_sub:
        return  # No force sub set, ignore
    force_sub = int(force_sub)

    # Check join request mode from DB
    join_req_mode = await get_setting('join_request_enabled') or False
    jr_access     = await get_setting('join_request_access') or False
    if bool(JOIN_REQUEST_ENABLE) or join_req_mode or jr_access:
        try:
            invite = await client.create_chat_invite_link(
                chat_id=force_sub, creates_join_request=True)
            button_url = invite.invite_link
        except Exception:
            button_url = getattr(client, 'invitelink', None) or f"https://t.me/c/{str(force_sub).replace('-100', '')}"
    else:
        button_url = getattr(client, 'invitelink', None)
        if not button_url:
            try:
                button_url = (await client.get_chat(force_sub)).invite_link
            except Exception:
                button_url = f"https://t.me/c/{str(force_sub).replace('-100', '')}"

    # In join request access mode — check if user already sent a request
    # by looking at join_requests DB. If on_chat_join_request already fired,
    # DB will have it and is_subscribed would have returned True already.
    # If we're here, it means DB doesn't have it yet — save it now so next
    # Try Again works. We use get_chat_member to detect pending status.
    if jr_access or join_req_mode:
        user_id = message.from_user.id
        try:
            # If user sent a join request, Telegram raises UserNotParticipant
            # but on_chat_join_request fires separately. So we just save to DB
            # proactively so the NEXT Try Again works instantly.
            from database.database import add_join_request, database as _db
            existing = _db['join_requests'].find_one({'user_id': user_id})
            if not existing:
                # Not in DB yet — user hasn't sent request or handler hasn't fired
                # Show message asking them to send request first
                buttons_jr = [[InlineKeyboardButton("📢 Send Join Request", url=button_url)]]
                try:
                ...
                buttons_jr.append([InlineKeyboardButton("🔄 I Sent Request — Try Again", url=try_again_url)])
                await message.reply(
                    f"👋 <b>Hey {message.from_user.mention}!</b>\n\n"
                    f"🔒 <b>Access Restricted</b>\n\n"
                    f"1️⃣ Tap <b>Send Join Request</b> below\n"
                    f"2️⃣ Send the join request in the channel\n"
                    f"3️⃣ Come back and tap <b>I Sent Request — Try Again</b>\n\n"
                    f"<i>You don't need admin approval — just sending the request is enough!</i>",
                    reply_markup=InlineKeyboardMarkup(buttons_jr),
                    quote=True, disable_web_page_preview=True
                )
            else:
                # In DB but is_subscribed returned False — race condition
                # Grant access by showing welcome (shouldn't normally happen)
                await message.reply(
                    f"✅ <b>Request found!</b> Tap Try Again.",
                    reply_markup=InlineKeyboardMarkup([[
