...
async def not_joined(client: Client, message: Message):
    ...
    # In join request access mode — save user to DB immediately and grant access
    # We trust they sent a request since they used the join request link
    if jr_access or join_req_mode:
        ...
        try:
            ...
            if not existing:
                # First time — show join button
                buttons_jr = [
                    [InlineKeyboardButton("📢 Send Join Request", url=button_url)],
                    [InlineKeyboardButton("✅ I Sent Request", callback_data=f"jr_done_{user_id}")]
                ]
                await message.reply(
                    f"👋 <b>Hey {message.from_user.mention}!</b>\n\n"
                    f"🔒 <b>Access Restricted</b>\n\n"
                    f"1️⃣ Tap <b>Send Join Request</b>\n"
                    f"2️⃣ Send the join request in the channel\n"
                    f"3️⃣ Come back and tap <b>✅ I Sent Request</b>\n\n"
                    f"<i>You don't need admin approval — just sending the request is enough!</i>",
                    reply_markup=InlineKeyboardMarkup(buttons_jr),
                    quote=True, disable_web_page_preview=True
                )
            else:
                # Already in DB but is_subscribed returned False — race condition
                # Just grant them access by redirecting to start
                await message.reply(
                    f"✅ <b>Request verified!</b>\n\nSend /start to access the bot.",
                )
            return
        except Exception:
            pass
...
# ─────────────────────────────────────────────────────────────────────────────
#  Join Request Done callback — user taps "I Sent Request"
# ─────────────────────────────────────────────────────────────────────────────
@Bot.on_callback_query(filters.regex(r"^jr_done_(\d+)$"))
async def jr_done_callback(client, query):
    from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
    user_id = query.from_user.id

    # Save to DB — grant access
    ...
    await query.message.edit_text(
        ...
        "Tap below to start using the bot!",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("🚀 Start Bot", url=f"https://t.me/{client.username}?start=start")
        ]])
    )
