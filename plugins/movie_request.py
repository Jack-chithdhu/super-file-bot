...
async def req_callback(client: Client, query: CallbackQuery):
    ...
    if action == "fulfill":
        await query.answer("Send the file link or getfiles ID.")
        try:
            link_msg = await client.ask(
                ...
                text=(
                    ...
                    f"Send one of:\n"
                    f"• A batch link: <code>https://t.me/...?start=xxx</code>\n"
                    f"• A getfiles ID: <code>{req_id}</code>\n\n"
                    f"Or send /skip to fulfill without a file link."
                ),
        ...
        if raw.startswith("/skip"):
            ...
            file_button = InlineKeyboardButton("📥 Get Files", url=raw)
            note = "Your files are ready!"
        else:
            getfiles_id = raw.upper()
            file_button = InlineKeyboardButton("📥 Get Files", callback_data=f"gf_back_{getfiles_id}")
            note = "Your files are ready!"
