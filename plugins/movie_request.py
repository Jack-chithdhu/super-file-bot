...
async def req_callback(client: Client, query: CallbackQuery):
    ...
    if action == "fulfill":
        # Show collections as inline buttons — no text input needed
        await query.answer()
        from database.database import collection_get_all
        collections = await collection_get_all()

        if not collections:
            await client.send_message(
                query.from_user.id,
                f"📥 <b>Fulfill Request</b>\n🆔 <code>{req_id}</code> — <b>{req['title']}</b>\n\n"
                f"No collections found.\nCreate one first: <code>/newcollection &lt;name&gt;</code>\n\n"
                f"Then click Fulfill again, or use /fulfill {req_id} to skip files.",
            )
            return

        buttons = [
            [InlineKeyboardButton(f"#{c['number']} — {c['name']}", callback_data=f"fulfill_col_{req_id}_{c['number']}")]
            for c in collections
        ]
        buttons.append([InlineKeyboardButton("⏭ Skip (no files)", callback_data=f"fulfill_col_{req_id}_skip")])

        await client.send_message(
            query.from_user.id,
            f"📥 <b>Fulfill Request</b>\n🆔 <code>{req_id}</code> — <b>{req['title']}</b>\n\n"
            f"Select the collection to link:",
            reply_markup=InlineKeyboardMarkup(buttons)
        )
        return  # Handled by fulfill_col_ callback below
...
# ─────────────────────────────────────────────────────────────────────────────
#  Collection selection callback during fulfill
# ─────────────────────────────────────────────────────────────────────────────
@Bot.on_callback_query(filters.regex(r"^fulfill_col_(.+)_(skip|\d+)$"))
async def fulfill_col_cb(client, query: CallbackQuery):
    all_admins = list(set(ADMINS + await get_bot_admins()))
    if query.from_user.id not in all_admins:
        await query.answer("❌ Not an admin.", show_alert=True)
        return
