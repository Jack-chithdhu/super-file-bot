...
@Bot.on_message(filters.private & filters.text & ~filters.command([
    ...
    'addfile','getfiles','listfiles','deletefile','newcollection','listcollections','filestore',
    ...
    if action == "fulfill":
        ...
        if raw.startswith("/skip"):
            ...
            try:
                ...
                if col:
                    file_button = InlineKeyboardButton("📥 Get Files", callback_data=f"gf_back_{col_number}")
                    note = f"Your files are ready! Browse them below. 🎊"
                else:
                    await link_msg.reply(f"❌ Collection #{col_number} not found. Fulfilling without file link.")
                    file_button = None
                    note = "Your request has been fulfilled!"
            except ValueError:
                await link_msg.reply("❌ Invalid. Fulfilling without file link.")
                file_button = None
                note = "Your request has been fulfilled!"
