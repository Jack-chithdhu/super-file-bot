...
@Bot.on_message(filters.private & filters.text & ~filters.command([
    ...
    'addfile','getfiles','listfiles','deletefile','newcollection','listcollections','linkcollection','linkfile','filestore',
...
async def req_callback(client: Client, query: CallbackQuery):
    ...
    if action == "fulfill":
        await query.answer("Send the collection number.")
        try:
            from database.database import collection_get_by_number, collection_get_all
            collections = await collection_get_all()
            if collections:
                lines = ["📦 <b>Collections</b>\n"]
                for c in collections:
                    lines.append(f"  <b>#{c['number']}</b> — {c['name']}")
                lines.append("\nSend the collection number for this request.\nOr /skip to fulfill without files.")
                ask_text = "\n".join(lines)
            else:
                ask_text = (
                    f"📥 <b>Fulfill Request</b>\n"
                    f"🆔 <code>{req_id}</code> — <b>{req['title']}</b>\n\n"
                    f"No collections found. Send /skip to fulfill without files\nor create a collection first with /newcollection."
                )
            ...
            await client.send_message(query.from_user.id, "⏰ Timed out. Use /fulfill command instead.")
            return

        raw = link_msg.text.strip()

        if raw.startswith("/skip"):
            file_button = None
            note = "Your request has been fulfilled!"
        else:
            try:
                col_number = int(raw)
                from database.database import collection_get_by_number, link_collection_to_request
                col = await collection_get_by_number(col_number)
                if col:
                    # Auto-link collection to this request ID
                    await link_collection_to_request(col_number, req_id)
                    # User uses their own request ID to get files
                    file_button = InlineKeyboardButton("📥 Get Files", callback_data=f"gf_req_{req_id}")
                    note = "Your files are ready! 🎊"
                else:
                    await link_msg.reply(f"❌ Collection #{col_number} not found. Fulfilling without file link.")
                    file_button = None
                    note = "Your request has been fulfilled!"
            except ValueError:
                await link_msg.reply("❌ Invalid number. Fulfilling without file link.")
                file_button = None
                note = "Your request has been fulfilled!"
        ...
        try:
            markup = InlineKeyboardMarkup([[file_button]]) if file_button else None
            await client.send_message(
                chat_id=req['user_id'],
                text=(
                    f"╔══════════════════════╗\n"
                    ...
                    f"🎬 Title: <b>{req['title']}</b>\n\n"
                    f"{note} 🎊"
                    + ("\n\nTap the button below to get your files:" if file_button else "")
                ),
                reply_markup=markup
            )
        except Exception:
