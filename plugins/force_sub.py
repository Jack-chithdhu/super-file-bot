...
async def check_subscription(client: Client, user_id: int) -> bool:
    """Return True if the user passes the force-sub gate.

    Short-circuits immediately (True) for:
    - admins
    - premium users
    - when no force-sub channel is configured
    - join_request_access mode: user has a pending join request in DB

    Returns False only when the channel is set and the user is
    genuinely not a member.
    """
    # Admins always pass — check before any I/O
    ...
    # join_request_access: a pending join request counts as "subscribed"
    ...
    try:
        ...
        log.warning(f"check_subscription get_chat_member error: {e}")
        # Unknown error — fail open so users aren't locked out by Telegram issues
        return True
