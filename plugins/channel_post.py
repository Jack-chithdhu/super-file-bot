...
@Bot.on_message(filters.private & filters.user(ADMINS) & ~filters.command([
    ...
    'addfile','getfiles','listfiles','deletefile','newcollection','listcollections','linkcollection','linkfile','filestore','cancel','linkstats',
    'clearjoinrequests'
]), group=1)
async def channel_post(client: Client, message: Message):
    ...
    # Skip plain text messages — these are likely settings input (channel IDs, numbers, etc.)
    # Only store actual media/files, not text
    if message.text and not message.media:
        return

    # Skip if owner is in a settings flow (e.g. setting QR image, welcome message etc.)
    from config import OWNER_ID
    if message.from_user.id == OWNER_ID:
        # Check if bot is waiting for DB channel forward
        if hasattr(client, 'waiting_db_forward') and OWNER_ID in client.waiting_db_forward:
            return
        # Check if bot is waiting for QR photo
        if hasattr(client, 'waiting_qr') and OWNER_ID in client.waiting_qr:
            return
        # Check pyromod listener state
        for attr in ['listening', '_listening', 'listen_handlers', '_listeners', 'listeners']:
            try:
                listeners = getattr(client, attr, {})
                if listeners and message.chat.id in listeners:
                    return
            except Exception:
                pass
