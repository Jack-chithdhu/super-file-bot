...
async def is_subscribed(filter, client, update):
    ...
    from database.database import get_setting as _get_setting, database as _db
    ...
    log.info(f"is_subscribed: user={user_id} jr_access={jr_access} jr_enabled={jr_enabled}")

    if jr_access or jr_enabled:
        try:
            doc = _db['join_requests'].find_one({'user_id': user_id})
            log.info(f"is_subscribed: join_requests doc for {user_id} = {doc}")
            if doc:
                return True
        except Exception as e:
            log.warning(f"is_subscribed DB check error: {e}")
            pass
    ...
    try:
        ...
        log.warning(f"is_subscribed check error for {user_id}: {e}")
        if 'Peer id invalid' in str(e) or 'peer_id_invalid' in str(e).lower() or 'ID not found' in str(e):
            try:
                ...
                log.warning(f"Peer resolve failed: {e2}")
                return False  # Block user if we can't verify
        else:
            return False  # Block user on unknown errors
    # Check member status
    if member.status in [
