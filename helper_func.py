...
async def is_subscribed(filter, client, update):
    ...
    # Check join request mode — if enabled, user just needs to send request
    from database.database import get_setting as _get_setting
    join_req_mode = await _get_setting('join_request_enabled') or False
    if join_req_mode:
        # Check if user has sent a join request (pending or approved)
        try:
            from database.database import add_join_request
            # If user is already in DB as having requested — allow them
