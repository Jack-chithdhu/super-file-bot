...
class Bot(Client):
    ...
    async def start(self):
        ...
        try:
            from plugins.getfiles import addfile_cmd
            self.LOGGER(__name__).info("✅ getfiles plugin loaded OK")
        except Exception as e:
            self.LOGGER(__name__).error(f"❌ getfiles plugin FAILED to load: {e}")
