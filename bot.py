...
class Bot(Client):
    ...
    async def start(self):
        ...
        # ── DB channel check ───────────────────────────────────────────────
        try:
            # Manually store peer in session so Pyrogram can resolve it
            from pyrogram.raw import functions, types as raw_types
            channel_id = abs(CHANNEL_ID)
            if str(CHANNEL_ID).startswith("-100"):
                channel_id = int(str(CHANNEL_ID)[4:])
            await self.invoke(functions.channels.GetMessages(
                channel=await self.resolve_peer(CHANNEL_ID),
                id=[raw_types.InputMessageID(id=1)]
            ))
        except Exception:
            pass

        try:
            test = await self.send_message(chat_id=CHANNEL_ID, text="✅ Bot started!")
            self.db_channel = test.chat
            await test.delete()
        except Exception as e:
            self.LOGGER(__name__).warning(str(e))
            self.LOGGER(__name__).warning(
                f"Could not send to DB Channel. Continuing anyway. CHANNEL_ID: {CHANNEL_ID}"
            )
            self.db_channel = type('obj', (object,), {'id': CHANNEL_ID})()
