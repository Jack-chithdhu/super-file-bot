...
# Optional: Set SESSION_STRING to persist Pyrogram session across redeploys
# Generate using: python3 -c "from pyrogram import Client; Client(':memory:', api_id=APP_ID, api_hash=API_HASH, bot_token=TG_BOT_TOKEN).run()"
...
FORCE_SUB_INVITE_LINK = os.environ.get("FORCE_SUB_INVITE_LINK", "")
...
TG_BOT_WORKERS = int(os.environ.get("TG_BOT_WORKERS", "1"))  # Default 1 to save Railway credits

# ── Low Power Mode ────────────────────────────────────────────────────────────
# Set LOW_POWER=true in env to reduce RAM/CPU usage for free tier hosting
LOW_POWER = os.environ.get("LOW_POWER", "false").lower() == "true"
if LOW_POWER:
    TG_BOT_WORKERS = 1  # Reduce workers to minimum

# ── Survival Mode ─────────────────────────────────────────────────────────────
# Set SURVIVAL_MODE=true in env when credits are low.
# In survival mode ONLY file delivery works — no captcha, no support chat,
# no requests, no announcements, no premium checks, no force subscribe.
# Bot uses minimum possible resources to stay alive just for file links.
SURVIVAL_MODE = os.environ.get("SURVIVAL_MODE", "false").lower() == "true"
if SURVIVAL_MODE:
    TG_BOT_WORKERS = 1
