...
# Simple TTL cache for settings (30 second cache)
_settings_cache = {}
...
async def get_setting(key: str):
    ...
    if now - _settings_cache_time < 30 and _settings_cache:
...
async def set_setting(key: str, value):
    ...
    # Invalidate cache so next get_setting reads fresh data
    _settings_cache = {}
    _settings_cache_time = 0
...
async def add_join_request(user_id: int):
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, lambda: join_req_data.update_one(
        {'user_id': user_id},
...
async def get_pending_requests(limit: int = 50) -> list:
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, lambda: list(join_req_data.find({'status': 'pending'}).limit(limit)))
...
getfiles_data = database['getfiles_store']
try:
    getfiles_data.create_index([('request_id', 1), ('season', 1), ('quality', 1)], unique=True)
except Exception:
    pass

async def getfiles_add(request_id: str, season: int, quality: str, batch_link: str, file_count: int, title: str = ""):
    """Store or update a batch link for a request_id / season / quality."""
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, lambda: getfiles_data.update_one(
        {'request_id': request_id, 'season': season, 'quality': quality},
        {'$set': {
...
async def getfiles_get_seasons(request_id: str) -> list:
    """Return sorted list of season numbers stored for a request_id."""
    loop = asyncio.get_event_loop()
    docs = await loop.run_in_executor(None, lambda: list(
        getfiles_data.find({'request_id': request_id}, {'season': 1})
    ))
    return sorted(set(d['season'] for d in docs))

async def getfiles_get_qualities(request_id: str, season: int) -> list:
    """Return list of quality strings for a request_id + season."""
    loop = asyncio.get_event_loop()
    docs = await loop.run_in_executor(None, lambda: list(
        getfiles_data.find({'request_id': request_id, 'season': season}, {'quality': 1})
    ))
    return [d['quality'] for d in docs]

async def getfiles_get_link(request_id: str, season: int, quality: str) -> dict:
    """Return the stored doc for a specific request/season/quality."""
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, lambda: getfiles_data.find_one(
        {'request_id': request_id, 'season': season, 'quality': quality}
    ))

async def getfiles_delete_quality(request_id: str, season: int, quality: str):
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, lambda: getfiles_data.delete_one(
        {'request_id': request_id, 'season': season, 'quality': quality}
    ))

async def getfiles_delete_season(request_id: str, season: int):
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, lambda: getfiles_data.delete_many(
        {'request_id': request_id, 'season': season}
    ))

async def getfiles_delete_all(request_id: str):
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, lambda: getfiles_data.delete_many(
        {'request_id': request_id}
    ))

async def getfiles_list(request_id: str) -> list:
    """Return all entries for a request_id sorted by season then quality."""
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, lambda: list(
        getfiles_data.find({'request_id': request_id}).sort([('season', 1), ('quality', 1)])
    ))


# ════════════════════════ COLLECTION NUMBERS ══════════════════════════════════

collection_num_data = database['collection_numbers']

def _get_next_collection_number() -> int:
    """Get and increment the global collection counter."""
    result = collection_num_data.find_one_and_update(
        {'_id': 'counter'},
        {'$inc': {'seq': 1}},
        upsert=True,
...
async def collection_create(name: str, created_by: int) -> int:
    """Create a new collection with an auto-increment number. Returns the number."""
    loop = asyncio.get_event_loop()
    num = await loop.run_in_executor(None, _get_next_collection_number)
    await loop.run_in_executor(None, lambda: collection_num_data.update_one(
        {'number': num},
        {'$set': {
            'number':     num,
            'name':       name,
...
async def collection_get_by_number(number: int) -> dict:
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, lambda: collection_num_data.find_one({'number': number}))

async def collection_get_all() -> list:
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, lambda: list(
        collection_num_data.find({'number': {'$exists': True}}).sort('number', 1)
    ))

async def collection_delete(number: int):
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, lambda: collection_num_data.delete_one({'number': number}))


# ════════════════════════ COLLECTION LINKS ════════════════════════════════════
# Links a collection number to a request ID so users can use /getfiles <req_id>

collection_links_data = database['collection_links']

async def link_collection_to_request(collection_number: int, request_id: str):
    """Link a collection number to a request ID."""
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, lambda: collection_links_data.update_one(
        {'request_id': request_id},
        {'$set': {'request_id': request_id, 'collection_number': collection_number}},
        upsert=True
    ))

async def get_collection_for_request(request_id: str) -> int:
    """Get the collection number linked to a request ID. Returns None if not linked."""
    loop = asyncio.get_event_loop()
    doc = await loop.run_in_executor(None, lambda: collection_links_data.find_one({'request_id': request_id}))
    return doc['collection_number'] if doc else None


# ════════════════════════ COLLECTION ↔ REQUEST LINKS ═════════════════════════

collection_links_data = database['collection_links']
collection_links_data.create_index('request_id', unique=True)

async def link_collection_to_request(collection_number: int, request_id: str):
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, lambda: collection_links_data.update_one(
        {'request_id': request_id},
        {'$set': {'collection_number': collection_number, 'linked_at': datetime.utcnow()}},
        upsert=True
    ))

async def get_collection_for_request(request_id: str) -> int | None:
    loop = asyncio.get_event_loop()
    doc = await loop.run_in_executor(None, lambda: collection_links_data.find_one({'request_id': request_id}))
    return doc['collection_number'] if doc else None

async def unlink_collection_from_request(request_id: str):
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, lambda: collection_links_data.delete_one({'request_id': request_id}))
